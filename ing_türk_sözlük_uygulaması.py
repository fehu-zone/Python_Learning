...
def kelimeEkle(kelime,kelimeler):
	if kontrol(kelime,kelimeler): #ekleyeceğimiz kelimeyi yazdığımız kontrol fonksiyonundan kontrol ediyor 
		mevcut_anlamlar = set(kelimeler[kelime]) # kelimeleri set(küme şeklinde yazdırıyor)
		...
		if yeni_giriş.lower()=="e": # e şıkkını seçerse
			yeni_anlamlar= input("Girmek istediğiniz anlamları aralarına virgül koyarak yazın: ") # e şıkkını seçerse kullanıcıdan yeni bir anlam yazmasını isteyecek
			yeni_anlamlar_bölünmüş= set(yeni_anlamlar.split(",")) #yeni anlamı olan programı split komutuyla ayrı bir dizin olarak yazacak araya virgül koyacak
