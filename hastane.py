...
class Hastane():
	...
	def getDoktorlar(self):
		print("=========Doktorlarımız=========")
		for doktor in self.__doktorlar: #doktorlar en baştaki Hastahane classının içinde. Yeni bir get fonksiyonu açarak doktorlara ait bilgileri giriyoruz
			print("""
			...
			print("="*50)
			# print içindeki isim, soyad, telefon, bölüm

	def doktorEkle(self,yeni_doktor):
		self.__doktorlar.append(yeni_doktor) # baştaki Hastane Classında bulunan doktorlar parametresini, eklediğimiz yeni_doktor parametresine eklemiş olduk 
		print("Yeni doktor eklendi.") #eklediğimiz yeni doktor getDoktorlar fonksiyonuna gitti oradaki bilgileri okudu ve yeni doktoru getDoktorlar kısmına ekledi 
...
class Birey(): # hasta da doktor da bireydir. Hasta için ayrı bir class açtık 
...
class Doktor(Birey): #getDoktorlar fonksiyonundaki doktorları tanımlamak için bu classı açtık 

	doktor_sayısı= 0


	def __init__(self,isim,soyad,telefon,bölüm):
		super().__init__(isim,soyad,telefon) #super metodu ile isim,soyad,telefon, bölüm gibi Birey fonksiyonuna ait parametreleri ya aynı şekilde kullanıyoruz ya da değiştirme yapabiliyoruz
		Doktor.doktor_sayısı_artır() #doktor sayısı artırılmak istendiğinde bu çalışacak
		self.__bölüm= bölüm
...
class Hasta(Birey): # Birey fonksiyonunda özelliklerini belirttik 
	pass
